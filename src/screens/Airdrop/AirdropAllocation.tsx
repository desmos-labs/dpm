...
export const AirdropAllocation: React.FC<Props> = ({ navigation, route }) => {
  ...
  const { loading, allocations, allottedCoins, error, allClaimed, fetchAllocations } =
    useFetchAllottedDsm(params.address);
  ...
  useEffect(
    () => navigation.addListener('focus', fetchAllocations),
    [fetchAllocations, navigation]
  );

  const renderAllocation = useCallback(
    ({ item }: ListRenderItemInfo<Allocation>) => {
      const { chainName, claimed, amount } = item;
      return (
        <View>
          <View style={styles.allocation}>
            <Image source={require('../../assets/tick_orange.png')} style={styles.allocationTick} />
            {item.type === AllocationType.Staking ? (
              ...
              <Typography.Body1 style={styles.allocationText}>{chainName} LP</Typography.Body1>
            )}
            <Typography.Body1
              style={[styles.allocationAmount, claimed ? styles.allocationClaimed : null]}
            >
    ...
    [
      styles.allocation,
      styles.allocationTick,
      styles.allocationText,
      styles.allocationAmount,
      styles.allocationClaimed,
      styles.allocationAddress,
      t,
    ]
  );
    ...
    <StyledSafeAreaView topBar={<TopBar stackProps={{ navigation }} />}>
      <View style={styles.allocationContainer}>
        <Image
          ...
          resizeMode="contain"
        />
        <Typography.Body1 style={styles.allocationIsText}>
          {t('your allocation is')}
        </Typography.Body1>
        {error === null ? (
          <Typography.H1 ellipsizeMode="middle" numberOfLines={1}>
            {loading ? '...' : `${allottedCoins} DSM`}
          </Typography.H1>
        ) : (
          <Typography.Body>{`${t('an error occurred')} ${error}`}</Typography.Body>
        )}
      </View>
      <View style={styles.allocationDetails} onStartShouldSetResponder={() => true}>
        <Typography.Body1 style={styles.address} numberOfLines={1} ellipsizeMode="middle">
          {params.address}
        </Typography.Body1>
        <FlatList
        ...
        {allocations.length > 0 && (
          <Typography.Body1 style={styles.info}>* {t('airdrop accounts tip')}</Typography.Body1>
        )}
      </View>
