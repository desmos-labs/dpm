...
export const AirdropRequestFeeGrant: React.FC<Props> = ({
	navigation,
	route,
}) => {
	...
	return (
		<StyledSafeAreaView style={styles.root}>
			{error === undefined ? (
				...
				<DpmImage style={styles.loadingAnimation} source={'fail'} />
			)}

			<Typography.H4 style={styles.title}>{t('getting grant')}</Typography.H4>
			<Typography.Body style={styles.message}>
...
const useStyles = makeStyle((theme) => ({
	...
	loadingAnimation: {
		marginTop: '26%',
		height: Dimensions.get('window').height * 0.3 ?? 240,
		alignItems: 'center',
