...
export const AirdropClaimStatus: React.FC<Props> = ({ navigation, route }) => {
  ...
  const profileTaskCompleted = useMemo(() => profile !== null, [profile]);

  const accountConnectedTask = useMemo(() => chainLinks.length > 0, [chainLinks]);

  const grantTaskCompleted = useMemo(
    () => feeGrantRequestStatus?.type === FeeGrantStatus.Claimed,
    [feeGrantRequestStatus]
  );

  const canRequestFeeGrant = useMemo(
    () => feeGrantRequestStatus?.type === FeeGrantStatus.Claimable,
    [feeGrantRequestStatus]
  );
  ...
  const feeGrantUiText = useMemo(() => {
    ...
    if (feeGrantRequestStatus?.type === FeeGrantStatus.Error) {
      return feeGrantRequestStatus.error;
    }
    if (feeGrantRequestStatus?.type === FeeGrantStatus.Claimable) {
  ...
  return (
    ...
    >
      <Typography.Body1>{t('claim requirements')}</Typography.Body1>

      <View style={styles.tasksContainer}>
        <TaskItem text={t('get a grant title')} completed={grantTaskCompleted} />
        <Divider />
        <TaskItem
      ...
      <Typography.Body1 style={styles.feeGrantStatus}>{feeGrantUiText}</Typography.Body1>
      <FlexPadding flex={1} />
      {pendingRewards.available && (
        <Button style={styles.claimRewardsBtn} mode="outlined" onPress={claimRewards}>
...
const TaskItem: React.FC<TaskItemProps> = ({ completed, text }) => {
  ...
  const tickIcon = completed
    ? require('../../assets/tick_orange.png')
    : require('../../assets/tick_gray.png');

  return (
    <View style={styles.root}>
      <Image style={styles.tick} resizeMode="contain" source={tickIcon} />
      <Typography.Body1 style={styles.text}>{text}</Typography.Body1>
    </View>
  );
};
