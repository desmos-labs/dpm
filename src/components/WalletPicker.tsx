...
export type Props = {
    ...
    /**
     * Mnemonic used to generated the wallets.
     */
    mnemonic?: string,
    /**
...
export const WalletPicker: React.FC<Props> = ({
    ...
    const generateWalletFromHdPath = useCallback(async (hdPath: HdPath) => {
        try {
            ...
            if (ledgerTransport !== undefined) {
                wallets = await generateWalletsFromLedger(ledgerTransport, ledgerApp!, addressPrefix, [hdPath]);
            } else {
    ...
    const fetchWallets = useCallback(async (offset: number, limit: number) => {
        ...
        if (mnemonic !== undefined) {
            ...
            wallets = await generateWalletsFromLedger(ledgerTransport!, ledgerApp!, addressPrefix, hdPaths);
        }
        setGeneratingAddresses(false);
        return wallets;
    }, [addressPrefix, ledgerApp, ledgerTransport, mnemonic, selectedHdPath.coinType, setGeneratingAddresses]);
