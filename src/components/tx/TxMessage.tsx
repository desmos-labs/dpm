...
export const TxMessage: React.FC<Props> = ({ message }) => {
	...
	if (msg.typeUrl === MsgTypes.MsgSaveProfile) {
	...
	} else if (msg.typeUrl === MsgTypes.MsgSend) {
		if (msg.type === MessageType.DirectMsg) {
	...
	} else if (msg.typeUrl === MsgTypes.MsgMultiSend) {
		if (msg.type === MessageType.DirectMsg) {
	...
	} else if (msg.typeUrl === MsgTypes.MsgVote) {
		if (msg.type === MessageType.DirectMsg) {
	...
	} else if (msg.typeUrl === MsgTypes.MsgDelegate) {
		if (msg.type === MessageType.DirectMsg) {
	...
	} else if (msg.typeUrl === MsgTypes.MsgWithdrawDelegatorReward) {
		if (msg.type === MessageType.DirectMsg) {
			const decodedMessage = MsgWithdrawDelegatorReward.decode(
				msg.directMessage.value
			);
			return (
				<MessageWithdrawDelegatorRewards protobufMessage={decodedMessage} />
			);
		} else {
			return (
		...
		if (msg.type === MessageType.DirectMsg) {
			const decodedMessage = MsgLinkChainAccount.decode(
				msg.directMessage.value
			);
		...
		if (msg.type === MessageType.DirectMsg) {
			const decodedMessage = MsgUnlinkChainAccount.decode(
				msg.directMessage.value
			);
...
function getMessageType(msg: EncodeObject | Any | AminoMsg): MessageType {
	...
	} else if ((msg as EncodeObject).typeUrl !== undefined) {
		return MessageType.EncodeObject;
	} else {
...
function getMessageTypeUrl(
	msgType: MessageType,
	msg: EncodeObject | Any | AminoMsg
): string {
	switch (msgType) {
		case MessageType.DirectMsg:
