...
import {CallRequestEvent, CallRequestType, Events, ParsedCallRequest} from "../types/walletconnect";
...
export const WalletContextProvider: React.FC = ({children}) => {
    ...
    const onCallRequest = useCallback(async (event: CallRequestEvent) => {
        if (event.request) {
            ...
            if (parsed === null) {
                ...
                if (parsed.type === CallRequestType.GetAccounts) {
                    ...
                    const session = controller.sessions.find(session => session.id === event.sessionId);
                    const response = [];
                    if (session !== undefined) {
                        for (let sessionAccount of session.accounts) {
                            const chainAccount = await AccountSource.getAccount(sessionAccount);
