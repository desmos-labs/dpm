...
export class ProfileSource {
  ...
  async saveProfile(profile: DesmosProfile): Promise<[DesmosProfile, boolean]> {
    const storedProfile = await this.getProfile(profile.address);
  ...
  async deleteProfile(address: string) {
    ...
    if (profile !== null) {
      ...
      await RNFS.unlink(profileFile).catch((e) => console.error('unlink profileFile', e));
      // Delete the profile dir
      await RNFS.unlink(profileDir).catch((e) => console.error('unlink profileDir', e));
