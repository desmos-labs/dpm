...
async function signDirectTx(
	...
	if (!isOfflineDirectSigner(signer)) {
		throw new Error(
			"Can't sign, method direct is not supported from the wallet"
		);
	}
	...
	// Sign the document
	const signature = await directSigner.signDirect(
		account.address,
		finalSignDoc
	);
	return {
...
async function signAminoTx(
	...
	if (aminoSigner.signAmino === undefined) {
		throw new Error(
			"Can't sign, method amino is not supported from the wallet"
		);
	}
