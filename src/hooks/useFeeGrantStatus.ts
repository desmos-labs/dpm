...
export default function useFeeGrantStatus(desmosAddress: string, externalAddress: string) {
  ...
  const [feeGrantRequestStatus, setFeeGrantRequestStatus] = useState<FeeGrantRequest | undefined>(
    undefined
  );
  ...
  useEffect(() => navigation.addListener('focus', fetchStatus), [fetchStatus, navigation]);
