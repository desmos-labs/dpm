...
export type FeeGrantRequest =
	| FeeGrantRequestClaimed
	| FeeGrantRequestClaimable
	| FeeGrantRequestError;

export default function useFeeGrantStatus(
	desmosAddress: string,
	externalAddress: string
) {
	const [loading, setLoading] = useState(true);
	const [feeGrantRequestStatus, setFeeGrantRequestStatus] = useState<
		FeeGrantRequest | undefined
	>(undefined);
	...
	const fetchStatus = useCallback(async () => {
		...
		try {
			const result = await AirdropApi.feeGrantStatus(
				desmosAddress,
				externalAddress
			);
