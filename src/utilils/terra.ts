...
export default class TerraLedgerApp implements LedgerApp {
  ...
  async publicKey(path: Array<number>): Promise<PublicKeyResponse | ErrorResponse> {
    ...
    return {
      compressed_pk:
        response.compressed_pk !== undefined ? Buffer.from(response.compressed_pk.data) : undefined,
  ...
  async sign(path: Array<number>, message: string): Promise<SignResponse | ErrorResponse> {
    ...
    return {
      ...
      error_message: response.error_message,
      signature:
        response.signature !== undefined ? Buffer.from(response.signature.data) : undefined,
    };
  }
}
